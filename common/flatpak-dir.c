...
static void
add_ref_if_match (const char            *refspec,
                  const char            *opt_name,
                  ...
                  GPtrArray             *matched_refs)
{
  ...
  if (flags & FIND_MATCHING_REFS_FLAGS_KEEP_REMOTE)
    g_ptr_array_add (matched_refs, g_strdup (refspec));
  else
                    ...
                    GError      **error)
{
  ...
  while (g_hash_table_iter_next (&hash_iter, &key, NULL))
    {
      add_ref_if_match (key, opt_name, opt_branch, opt_arch, kinds, flags, matched_refs);
    }
  ...
  for (i = 0; i < n; i++)
    {
      ...
      add_ref_if_match (ref, opt_name, opt_branch, opt_arch, kinds, flags, matched_refs);
    }
